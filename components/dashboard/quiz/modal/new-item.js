...
const NewItemModal = ({ onClose }) => {
    const { columns, createItem } = useBoards();
        ...
        >
            {formik => (
                <div className="modal w-full mx-auto rounded-md p-6 dark:bg-darkGrey md:p-8">
                    <h1 className="heading-lg mb-6">Add New Item</h1>
                    <Form>
                        ...
                        <button type="submit" className="mt-6 w-full bg-mainPurple text-white text-base rounded-full p-2 transition duration-200 hover:bg-mainPurpleHover">+ Add New Item</button>
                    </Form>
                </div>
            )}
